...
//creating a function to test the safety
void Safety(vector<vector<int>> allocation, vector<vector<int>> request, vector<int> available, vector<int> processId) 
{  // vector<vector<int>>== 2d vector or 2d dynamic array

    int safe[n];  // n == number of processes
    int k = 0;
    int completed[n] = {0};
    int finish = 0;

    vector<int> total(m); // m == number of resources
    int sumr = 0;
    for (int j = 0; j < m; j++) // j selects resource to do sum of its allocations
    {
        for (int i = 0; i < n; i++)
        {
            sumr += allocation[i][j]; // allocating the resoruces.
        }
        total[j] = available[j] + sumr; // after that, checking the available resources
        sumr = 0;
    }
    while (finish != n)
    {
        ...
        { // traversing the processes
            ...
            {
                int j = 0;
                while (j < m && request[i][j] <= available[j]) // loop to read the request and available resoucres and to compare them
                {
                    j++; // after checking if the resource is available it is allocated and then the value of j is increased by 1 so that to indicate the resources are allocated
                }
                if (j == m)
                { // request satisfied
                    safe[k++] = processId[i]; 
